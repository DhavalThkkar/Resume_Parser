...
def extract_skills_list():
    all_skills_endpoint = "https://emsiservices.com/titles/versions/latest/titles/mappings" # List of all skills endpoint
    auth = "Authorization: Bearer " + access_token # Auth string including access token from above
    headers = {'authorization': auth} # headers
    query = {'q': 'Data Scientist', 'limit':'5'}
    response = requests.request("GET", all_skills_endpoint, 
                                headers=headers, params=query) # response
    response = response.json() # the data
